...
| Entropie finale | `3.9055` bits | `3.9886` bits | Résultat de simulation, pas température matérielle |
| Plage du taux ETH interne | `0.0207` à `0.9385` bits/pas | `0.0120` à `0.9385` bits/pas | Variation de von Neumann par pas dans ce contrat — l’enveloppe de cryogénie virtuelle |
| `P_sig` de graphe | Oscillation déterministe, max `0.133231` | Oscillation déterministe, max `0.149935` | Tryperposition non contrôlée ; tension graphe finie dès le premier pas non nul (F3) |
| `P_sig` logique du sidecar | `0.1821619076 → 0.5893783934` | même sidecar algorithmique | Objet séparé du graphe de corrélation |
| `P_sig` tryperposition | `0.1879842865 → 0.1299670353` | `0.1879842865 → 0.120100` | Troisième canal Q × I × M, distinct des deux P sig bruts |
| Tension tryperposition maximale | `17.3834` | `18.7692` | Canal actif ; les deux profils partagent `alpha_0=1.0` (F2) |
| Pas de condition de collapse | `2, 3, 4, 5, 6, 7, 8, 9, 10` sans intervention | `2` à `10`, tous avec déphasage local appliqué | Conditions de scénario, pas effondrements matériels observés |

Le `P_sig` de graphe produit des cycles H1 finis à neuf pas sur onze — l’oscillation voulue du régime de tryperposition non contrôlée, reproductible à l’identique grâce à la graine figée. **F3&nbsp;: la référence n’est plus capturée au pas initial aveugle.** Elle est prise au **premier pas non nul** de chaque canal (`reference_steps` logge ce pas dans l’artefact)et le zéro initial reste publié. `graph_tension` reste `null` aux seuls pas où `graph_P_sig=0` (pas `0`, `7`, `10` dans cet artefact), chaque fois avec une raison explicite. Si aucun pas non nul n’existait sur toute la plage, `reference_graph_never_nonzero=true` le documenterait explicitement (R2). L’oscillation est conservée brute ; elle n’est ni lissée ni remplacée.
...
Les deux profils utilisent désormais la même abscisse `alpha_0=1.0` ; ils diffèrent uniquement par l’application du déphasage local déclaré dans le scénario de sensibilité. Ses dix conditions de tension tryperposition (pas 1 à 10) ont effectivement déclenché ce canal, tandis que la baseline reste observationnelle et ne déclenche aucune intervention. Les métriques LCT et ETH portent l’étiquette `pre_intervention_density`, tandis que l’état de sortie et l’entropie post-intervention restent dans des champs séparés. Le résultat ne démontre donc pas une protection, une correction ou un mécanisme cryogénique matériel ; il documente l’effet de cette hypothèse de scénario sur la simulation, ETH jouant le rôle d’enveloppe de cryogénie **virtuelle** autour du QPU logiciel.
